...
class Board:
    ...
    def wrongplace(self):
        """ calculates the distance of each tile from its goal position
        """
        num = 0
        for i in range(3):
            for j in range(3):
                ...
                if value != '0':
                    row, col = get_index_2d(GOAL_TILES, value)
                    if i != row:
                        num += 1
                    if j != col:
                        num += 1
        ...
        return num
#remember that "value" is a string, NOT a number!!!
 

def get_index_2d(GOAL_TILES, val):
    """ return the index of the goal value in a 2-d list
    """
    row, col = 0, 0
    for i in range(len(GOAL_TILES)):
        for j in range(len(GOAL_TILES[i])):
            if GOAL_TILES[i][j] == val:
                row, col = i, j
                break
    return row, col
