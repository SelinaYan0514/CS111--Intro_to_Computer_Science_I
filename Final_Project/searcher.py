...
def h1(state):
    """ returns the number of misplaced tiles in the board """
    return state.board.num_misplaced()

def h2(state):
    " returns the manhattan distance for current state"
    return state.board.wrongplace()
...
class GreedySearcher(Searcher):
    ...
    def add_state(self, state):
        """ Add a new state to the list of untested states """
        state_priority = self.priority(state)
        state_list = [[state_priority, state]]
        self.states += state_list

    def next_state(self):
        pair = max(self.states)
...
class AStarSearcher(Searcher):
    """ a class called AStarSearcher for searcher objects that perform A* search
    """
    def __init__(self, heuristic):
        """ constructs a new GreedySearcher object.
        """
    ...
    def add_state(self, state):
        ...
        priority = -1 * (heuristic_value + num_moves)
        state_list = [priority, state]
        self.states += [state_list]
